...
document.addEventListener('DOMContentLoaded', () => {
  ...
  qPortMeBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      // Save answers + instructions locally BEFORE auth/Firestore,
      // so progress survives if the popup closes during auth.
      ...
      // Firestore save succeeded — mark completed and clear pending state
      await new Promise(function (resolve) {
        chrome.storage.local.set({
      ...
      await new Promise(function (resolve) {
        chrome.storage.local.remove(['pending_instructions', 'pending_destination'], resolve);
      });
  ...
  portInstructionsBtn.addEventListener('click', async function () {
    // If questionnaire not completed yet, authenticate first then launch questionnaire.
    // Pre-auth ensures OAuth popup happens BEFORE questionnaire data exists,
    // so if the popup closes during OAuth, no form progress is lost.
    ...
    if (!completed) {
      try {
        ...
        setPortStatus('Connecting to Google\u2026');
        await ensureAuthenticated();
        setPortStatus('');
        portInstructionsBtn.disabled = false;
      } catch (authErr) {
        portInstructionsBtn.disabled = false;
        setPortStatus('Sign-in failed. Please try again.', true);
        return;
      }
  ...
  // ── Resume interrupted questionnaire save ──────────────────────────────
  // If the popup closed during OAuth/Firestore save, pending_instructions
  // will still be in storage. Complete the save now.
  chrome.storage.local.get(['pending_instructions', 'pending_destination', 'questionnaire_answers'], function (pending) {
    if (!pending.pending_instructions) return;
    (async function () {
      try {
        setPortStatus('Finishing save\u2026');
        var auth = await ensureAuthenticated();
        var answers = pending.questionnaire_answers || {};
        await saveInstructionsToFirestore(pending.pending_instructions, auth.userId, auth.idToken, auth.firebaseUid, answers);
        await new Promise(function (resolve) {
          chrome.storage.local.set({
            questionnaire_completed: true,
        ...
        setPortStatus('Could not finish saving. Click Port Me to retry.', true);
        // Clear pending state so it doesn't loop
        chrome.storage.local.remove(['pending_instructions', 'pending_destination']);
      }
    })();
  });
