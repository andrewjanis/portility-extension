...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function handleDestination(destination) {
    ...
    try {
      ...
      if (destination === 'save') {
        ...
        // Store text for auto-paste on destination tab — must complete before opening tab
        if (destination === 'claude' || destination === 'gemini' || destination === 'chatgpt') {
          await new Promise(function (resolve) {
            chrome.storage.local.set({ portility_pending_paste: finalText }, resolve);
          });
        }
  ...
  proChatBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      // Step 5: Call worker /summarize-pro
      ...
      const proResponse = await fetch(proxyBase + '/summarize-pro', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
