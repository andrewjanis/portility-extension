...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  // Inject into all frames to find viewer iframe and its download controls
  if (message.type === 'IFRAME_VIEWER_DOWNLOAD') {
    chrome.scripting.executeScript({
      target: { tabId: tabId, allFrames: true },
      world: 'MAIN',
      func: function () {
        var result = {
          frameUrl: location.href.substring(0, 300),
        ...
        // If this is the top frame, enumerate all iframes
        if (window === window.top) {
          var frames = document.querySelectorAll('iframe');
          for (var f = 0; f < frames.length; f++) {
            result.iframes.push(frames[f].src ? frames[f].src.substring(0, 300) : '(no src)');
          }
          return result;
        }

        // For child frames: scan for download controls
        var elems = document.querySelectorAll('button, [role="button"], a[href], [data-tooltip], [aria-label]');
        for (var i = 0; i < Math.min(elems.length, 50); i++) {
          var el = elems[i];
          var label = el.getAttribute('aria-label') || '';
          var tooltip = el.getAttribute('data-tooltip') || '';
          var text = (el.textContent || '').trim().substring(0, 60);
          var href = el.href || el.getAttribute('href') || '';
          var desc = [label, tooltip, text].filter(Boolean).join(' | ');

          if (el.tagName === 'A' && href) {
            result.links.push({ desc: desc.substring(0, 100), href: href.substring(0, 300) });
          } else if (desc) {
            result.buttons.push(desc.substring(0, 100));
          }

          if (/download/i.test(label) || /download/i.test(tooltip)) {
            result.downloadUrl = href || null;
            result.downloadButton = { label: label, tooltip: tooltip, tag: el.tagName };
            if (!href) {
              ...
              HTMLAnchorElement.prototype.click = function () {
                if (this.href) capturedUrl = this.href;
                return origAClick.apply(this, arguments);
              };
              try { el.click(); } catch (e) {}
              window.open = origOpen;
              HTMLAnchorElement.prototype.click = origAClick;
              if (capturedUrl) result.downloadUrl = capturedUrl;
            }
          }
        }
      ...
      if (results) {
        for (var r = 0; r < results.length; r++) {
          if (results[r].result) allFrames.push(results[r].result);
        }
      }

      // Find a frame with a download URL
      var downloadUrl = null;
      for (var df = 0; df < allFrames.length; df++) {
        if (allFrames[df].downloadUrl) {
          downloadUrl = allFrames[df].downloadUrl;
          break;
        }
      }
      ...
      if (downloadUrl) {
        fetch(downloadUrl, { credentials: 'include' })
          ...
          .catch(function (err) {
            sendResponse({ error: err.message, frames: allFrames });
          });
      } else {
        sendResponse({ error: 'no-download-url', frames: allFrames });
      }
    }).catch(function (err) {
      sendResponse({ error: err.message });
    });
